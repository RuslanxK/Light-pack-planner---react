...
const Trips = ({trips, bags, session}) => {
  ...
  return (
    ...
    <div class="trips">
    <Stack display={theme.grid} gap={4} gridTemplateColumns={theme.trips.columns} justifyContent="center">
   ...
   <Stack display={theme.flexBox} flexDirection={theme.row} alignItems={theme.center} justifyContent={theme.center}>
    ...
    <Typography component="h3" variant="span" fontWeight="500" sx={{color: theme.green, textDecoration: "underline", cursor: "pointer", "&:hover": {color: "#32cd32"}}} onClick={navigateToLatestBag}>{ trips.latestBag?.name.length > 10 ? `${trips.latestBag?.name.substring(0, 10)}...` : trips.latestBag?.name}</Typography>
    </Stack>
