...
const Trip = (props) => {
  ...
  return (
    <Stack display={theme.flexBox} justifyContent={theme.between} alignItems={theme.center} onMouseOver={() => setTripHover(true)} onMouseLeave={() => setTripHover(false)} borderRadius={theme.radius} height={theme.trips.height} onClick={NavigateToInnerTrip} boxShadow={theme.boxShadow} sx={{transition: theme.transition, cursor: "pointer", "&:hover": {boxShadow: theme.boxShadowHover}}}>
      ...
      <Typography height="25px" width="275px" pt={0.8} pb={1} backgroundColor="red" component="span" variant='span' fontSize="14px" display={theme.flexBox} alignItems={theme.center} justifyContent={theme.left}>{props?.tripData?.name?.length > 36? `${props.tripData.name.substring(0, 36)}..` : props?.tripData?.name} { tripHover ? <IconButton onClick={NavigateToInnerTrip} size='small' sx={{marginLeft: "5px"}}><NearMeOutlinedIcon sx={{fontSize: "17px"}} /></IconButton> : null }</Typography>
    </Stack>
  )
}
